...
import pistonAfter from "../imports/clean_piston.png";
...
import { ProductsProvider, useProducts } from "./data/products";

/**
 * Reliably scroll to a section id. On mobile, navigating from another page
 * means the target section may not be in the DOM yet when we try to scroll.
 * Retry every 100ms (up to 10 times) until the element exists.
 */
function scrollToId(id: string, attempts = 0) {
  ...
  if (el) {
    ...
    setTimeout(() => scrollToId(id, attempts + 1), 100);
  }
}

type Page = "home" | "gallery" | "product" | "shop" | "admin";
...
export default function App() {
  ...
  // Admin is hidden: it only opens when the URL ends with #admin.
  // Customers never see a link to it anywhere on the site.
  useEffect(() => {
    const check = () => {
      if (window.location.hash.toLowerCase() === "#admin") setPage("admin");
    };
    check();
    ...
    <ProductsProvider>
      <RouterCtx.Provider value={ctxValue}>
        ...
        >
          ...
          {page === "shop" && (
            <ShopPage onBack={() => go("home")} onOpen={openProduct} />
          )}
          {page === "admin" &&
            (adminUnlocked ? (
              <AdminPage onBack={leaveAdmin} />
            ) : (
              <AdminGate onUnlock={() => setAdminUnlocked(true)} onCancel={leaveAdmin} />
            ))}
          {page === "product" && (
            <ProductDetailPage
...
// ⚠️ Change this to your own secret passcode before publishing.
const ADMIN_PASSCODE = "piston2026";
...
function AdminGate({
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    if (pin === ADMIN_PASSCODE) onUnlock();
    else {
      setError(true);
      setPin("");
    }
  ...
  return (
    <main className="relative z-10 min-h-screen grid place-items-center px-6">
      <form
        onSubmit={submit}
        className="w-full max-w-sm rounded-3xl border border-white/10 bg-white/[0.03] p-8 text-center backdrop-blur"
      >
        <div className="mx-auto grid place-items-center h-14 w-14 rounded-full bg-[#ff6a18]/15 text-[#ff6a18]">
          <Cog className="h-6 w-6" />
        </div>
        <h1 style={{ fontFamily: "Bebas Neue", fontSize: 34 }} className="mt-5 leading-none">
          ADMIN ACCESS
        </h1>
        <p className="mt-2 text-sm text-white/45">Enter your passcode to manage products.</p>
        <input
          type="password"
          autoFocus
          value={pin}
          ...
          className={`mt-6 w-full rounded-xl border bg-black/40 px-4 py-3 text-center text-white outline-none transition-colors ${
            error ? "border-rose-500" : "border-white/10 focus:border-[#ff6a18]/60"
          }`}
        />
        {error && <p className="mt-2 text-xs text-rose-400">Wrong passcode. Try again.</p>}
        <button
          type="submit"
          className="mt-5 w-full rounded-full bg-gradient-to-r from-[#ff6a18] to-[#b34700] py-3.5 text-white shadow-[0_0_30px_rgba(255,106,24,0.45)] touch-manipulation"
        >
          Unlock
        </button>
        <button
                ...
                <a
                  ...
                  onClick={(e) => {
                    ...
                    if (n === "Gallery") {
                      ...
                      // Retry-until-present scroll handles the not-yet-rendered
                      // section after a page switch on mobile.
                      scrollToId(n.toLowerCase());
                    }
                  }}
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 grid lg:grid-cols-12 gap-8 lg:gap-12 items-center">
        ...
        >
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <button
              onClick={() => go("shop")}
              className="group inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-[#ff6a18] to-[#b34700] px-6 py-3.5 sm:px-7 sm:py-4 text-white shadow-[0_0_40px_rgba(255,106,24,0.45)] active:scale-95 transition-all touch-manipulation"
            >
              Shop Now
          ...
          <div className="relative aspect-square rounded-[32px] overflow-hidden border border-white/10 bg-gradient-to-br from-[#1a1a1a] to-[#0a0a0a] shadow-[0_30px_120px_rgba(255,106,24,0.25)]">
            ...
            >
              ...
              <img
                src={pistonClockFront}
                alt="Piston Clock"
                className="h-full w-full object-contain object-center drop-shadow-[0_20px_40px_rgba(255,106,24,0.35)]"
              />
            </motion.div>
            <div
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
        ...
        >
          <div className="aspect-[4/5] rounded-3xl overflow-hidden border border-white/10 relative">
            ...
            <div className="absolute bottom-6 left-6 right-6">
              ...
              >
                A CS STUDENT FROM KARNATAKA · 2 PIECES SO FAR
              </div>
            </div>
          </div>
        </motion.div>
        ...
        >
          ...
          <p className="mt-6 text-white/60 leading-relaxed">
            I'm a Computer Science student from Karnataka, where technology
        ...
        <div className="mt-10 md:mt-16 grid lg:grid-cols-2 gap-8 lg:gap-10 items-center">
          ...
          >
            ...
            <img
              ...
              // }`}
              className={`absolute inset-0 m-auto h-full w-full object-contain transition-transform duration-700 drop-shadow-[0_20px_50px_rgba(255,106,24,0.35)] ${
                    zoom ? "scale-150" : "scale-100 group-hover:scale-110"
              }`}
            />
...
function ReviewsBuyButton() {
  ...
  return (
    <button
      onClick={() => go("shop")}
      className="mt-7 inline-flex items-center gap-2 rounded-full border border-[#ff6a18]/40 bg-[#ff6a18]/10 px-6 py-3 text-white hover:bg-[#ff6a18]/20 transition touch-manipulation"
    >
      Shop All Clocks <ArrowRight className="h-4 w-4" />
    </button>
  );
}
    ...
    <section id="shop" className="relative z-10 py-16 md:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="text-center max-w-2xl mx-auto mb-8 md:mb-12">
          <SectionLabel icon={<Award />} text="THE COLLECTION" centered />
          <h2
            style={{ fontFamily: "Bebas Neue", fontSize: "clamp(34px,5vw,64px)" }}
            className="mt-4 leading-[0.95]"
          >
            {products.length} EDITIONS. ONE{" "}
            <span className="text-[#ff6a18]">OBSESSION.</span>
          </h2>
          <div className="mt-5 inline-flex items-center gap-3 rounded-full border border-white/10 bg-white/[0.03] px-5 py-2 text-xs tracking-widest text-white/60">
            <Truck className="h-3.5 w-3.5 text-[#ff6a18]" /> HANDMADE · HONEST PRICING · BUILT TO LAST
          </div>
        </div>
        ...
        <div className="mt-10 text-center">
          <button
            onClick={() => go("shop")}
            className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-[#ff6a18] to-[#b34700] px-7 py-4 text-white shadow-[0_0_40px_rgba(255,106,24,0.5)] hover:shadow-[0_0_60px_rgba(255,106,24,0.8)] transition touch-manipulation"
          >
...
function InstagramGallery() {
  const { go } = useRouter();
  const imgs = [pistonClockFront, pistonClockBack, pistonClockLeft, pistonClockRight];
  return (
    <section className="relative z-10 py-12 md:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
...
function FAQ() {
  const faqs = [
    ...
    },
    {
      ...
      a: "Absolutely. We use premium clock movements that operate with minimal noise and some clock mechanisms we use that produce ticking sound so that it reminds us of good olden days.",
    },
    {
    ...
    //   a: "Yes — engraving, dial color, finish and size customizations are available on request.",
    // },
    {
      q: "How long does delivery take?",
      a: "Standard orders: 7–10 business days.",
    },
    {
      q: "Is it heavy?",
      a: "It's a real piece of engine. Expect a solid, premium weight — typically 1.2–2 kg depending on the type of Product.",
    },
    {
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6">
        <div className="relative overflow-hidden rounded-[24px] md:rounded-[32px] border border-[#ff6a18]/30 bg-gradient-to-r from-[#ff6a18]/15 via-black to-[#ff6a18]/10 p-7 sm:p-10 text-center">
          ...
          <div className="relative">
            ...
            <button
              onClick={() => go("shop")}
              className="mt-7 inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-[#ff6a18] to-[#b34700] px-7 py-4 text-white shadow-[0_0_40px_rgba(255,106,24,0.5)] hover:shadow-[0_0_60px_rgba(255,106,24,0.8)] transition touch-manipulation"
            >
    ...
    <AnimatePresence>
      {show && (
        <motion.button
          ...
          exit={{ y: 80, opacity: 0 }}
          className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40 inline-flex items-center gap-2 sm:gap-3 rounded-full bg-gradient-to-r from-[#ff6a18] to-[#b34700] pl-2 pr-5 sm:pr-6 py-2 text-white shadow-[0_0_40px_rgba(255,106,24,0.6)] text-sm touch-manipulation"
        >
          <span className="grid place-items-center h-10 w-10 rounded-full bg-black/30">
            <Flame className="h-5 w-5" />
          </span>
          <span className="hidden sm:inline">Shop the Piston Clock Collection</span>
...
function JourneyShopButton() {
  ...
  return (
    <button
      onClick={() => go("shop")}
      className="rounded-full border border-white/15 bg-white/5 px-6 py-3 text-white/90 hover:bg-white/10 transition touch-manipulation"
    >
      Shop the Clocks
    </button>
  );
...
function GalleryPage() {
  const { go } = useRouter();
  const shots = [
    { label: "FRONT", img: pistonClockFront },
    { label: "BACK",  img: pistonClockBack },
    { label: "LEFT",  img: pistonClockLeft },
    { label: "RIGHT", img: pistonClockRight },
  ];
  return (
    <main className="relative z-10 pt-28 pb-16 md:pt-32 md:pb-24">
      <section className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        <div className="mt-6 flex items-end justify-between flex-wrap gap-6">
          ...
          <button
            onClick={() => go("shop")}
            className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-[#ff6a18] to-[#b34700] px-6 py-3 text-white shadow-[0_0_30px_rgba(255,106,24,0.45)] touch-manipulation"
          >
            Shop the Clocks <ArrowRight className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="mt-16 rounded-[32px] border border-[#ff6a18]/30 bg-gradient-to-r from-[#ff6a18]/15 via-black to-[#ff6a18]/10 p-10 text-center relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <button
              onClick={() => go("shop")}
              className="mt-6 inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-[#ff6a18] to-[#b34700] px-7 py-4 text-white shadow-[0_0_40px_rgba(255,106,24,0.5)] touch-manipulation"
            >
              Shop the Clocks <ArrowRight className="h-4 w-4" />
            </button>
          </div>
